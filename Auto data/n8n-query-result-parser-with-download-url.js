...
function pushStatus(payload, record, fallbackText, binary) {
  ...
  statusRecords.push({
    meta: {
      ...
      downloadUrl: record.downloadUrl || null, // ✅ 新增：保存下载链接
    },
...
const queriesWithFiles = statusRecords.filter(({ meta, binary }) => {
  ...
  const hasDownloadUrl = Boolean(meta.downloadUrl); // ✅ 新增：检查是否有下载链接
...
if (!resultRecords.length && queriesWithFiles.length > 0) {
  return queriesWithFiles.map(({ meta, binary }) => {
    ...
    // ✅ 新增：添加下载按钮（如果有下载链接但没有文件）
    let downloadSection = '';
    if (meta.downloadUrl && !binary?.data) {
    ...
    const output = {
      json: {
        ...
        downloadUrl: meta.downloadUrl || null, // ✅ 新增：输出下载链接
...
if (!resultRecords.length) {
  ...
  return targets.map(({ meta }) => ({
    json: {
      ...
      text: meta.text || '',
      queryInfo: meta.queryInfo || {},
      downloadUrl: meta.downloadUrl || null, // ✅ 新增：即使失败也传递下载链接
    },
  }));
}
...
resultRecords.forEach(({ record: res, context }, idx) => {
  ...
  const json = {
    ...
    filePresent: meta.filePresent ?? false,
    fileInfo: meta.fileInfo || null,
    downloadUrl: meta.downloadUrl || null, // ✅ 新增：传递下载链接
    resultIndex: idx + 1,
  ...
  if (requestedMerchants.size > 1) {
    requestedMerchants.forEach(merchantId => {
      if (!processedMerchants.has(String(merchantId))) {
        ...
        let downloadSection = '';
        if (meta.downloadUrl) {
          const fileName = `${merchantId}_result.csv`;
          downloadSection = generateDownloadButton(meta.downloadUrl, fileName);
        }
        ...
        // 检查是否已经为这个商户生成过输出
        const alreadyAdded = outputs.some(o => o.json.messageid === meta.messageid &&
