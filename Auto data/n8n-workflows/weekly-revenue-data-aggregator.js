...
// ========== 3. 全平台汇总 ==========
// 所有商户维度且游戏名="合计"的数据之和
const platformTotal = allRevenueData
  .filter(row => row['游戏名'] === '合计' && row['商户名'])
  .reduce((acc, row) => {
    return {
...
allRevenueData
  ...
  .forEach(row => {
    ...
    merchantMap.set(merchantName, {
      类型: '商户',
      商户名: merchantName,
      总投注USD: num(row['总投注USD']),
      总派奖USD: num(row['总派奖USD']),
      总GGR_USD: num(row['GGR-USD']),
      总局数: num(row['总局数']),
    });
  });
...
allRevenueData
  .filter(row => row['游戏名'] && row['游戏名'] !== '合计' && row['商户名'])
  .forEach(row => {
...
// ========== 6. 币种维度聚合 ==========
// 每种币种的所有商户数据之和（排除 游戏名="合计"）
const currencyMap = new Map();

allRevenueData
  .filter(row => row['货币'] && row['游戏名'] && row['游戏名'] !== '合计' && row['商户名'])
  .forEach(row => {
