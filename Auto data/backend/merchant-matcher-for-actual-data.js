...
if (firstInput.stat_type || firstInput.merchant || firstInput.game_id) {
  ...
  console.log("需要先获取商户映射数据，然后再进行匹配");
    ...
    if (data.stat_type && data.stat_type.includes('merchant') && data.merchant) {
      results.push({
        json: {
          ...data,
          isMatched: false,
          matchType: 'no_mapping_data',
          merchant_id: data.merchant,
          merchant_name: `未匹配_${data.merchant}`
        }
      });
    } else if (data.stat_type && data.stat_type.includes('game') && data.game_id) {
      results.push({
        json: {
          ...data,
          isMatched: false,
          ...
          game_name: `未匹配_${data.game_id}`
        }
      });
    }
  });
  
  console.log(`📊 返回 ${results.length} 个数据项（未匹配状态）`);
  return results;
}
