// 上月日期计算器
// 根据今日日期输出上月的日期范围 (YYYYMMDD-YYYYMMDD格式)
// 用于n8n工作流

const inputData = $input.all();

console.log('=== 计算上月日期范围 ===');
...
console.log('上月第一天:', lastMonthFirstDay.toISOString().split('T')[0]);
console.log('上月最后一天:', lastMonthLastDay.toISOString().split('T')[0]);
...
return {
  json: {
    ...
    lastMonth: lastMonth + 1, // 转换为1-12格式
    lastMonthName: ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'][lastMonth],
    today: today.toISOString().split('T')[0],
    currentYear: currentYear,
    currentMonth: currentMonth + 1,
    currentMonthName: ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'][currentMonth],
    calculation: {
