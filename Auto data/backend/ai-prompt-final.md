...
5. **查询ID**：非必有
   - 从betId字段获取投注ID
   - 如果betId字段为空，则不添加ID条件
   - 自动去除ID前缀，只保留纯数字

6. **查询货币**：非必有
   - 从currency字段获取货币类型
   - 如果currency字段为空，则不添加货币条件
   - 支持USDT、MXN、INR等货币类型
...
4. 默认使用provider = 'gp'条件
5. 使用LIMIT 100限制结果数量
6. 所有金额字段使用ROUND(CAST(amount AS DOUBLE), 2)格式
...
请严格按照以下JSON格式返回结果（所有字段值必须用中文）：
{
  ...
  "extractedInfo": {
    ...
    "merchantId": "商户ID（从merchantid字段获取）",
    "timeRange": "时间范围信息（中文）",
...
8. **商户ID必须从merchantid字段准确获取**
9. **SQL格式必须完全按照示例格式，包括字段顺序和格式**
10. **使用merchant字段进行查询，不是merchant_id字段**
11. **使用updated_at字段进行时间范围查询**
12. **时间范围格式：从00:00:00到23:59:59**
```
...
```json
{
  ...
  "extractedInfo": {
    "merchantName": "JWgame",
    "merchantId": "1754913084",
    "timeRange": "2025-10-16 00:00:00 至 2025-10-16 23:59:59",
    "gameInfo": "无",
    "userInfo": "无",
    "recordId": "无",
    "currency": "无"
  ...
  "suggestedQuestions": [],
  "canExecute": true,
  "reason": "成功提取商户ID和时间范围，可以生成有效查询",
  "generatedSQL": "SELECT\n  id,\n  uid,\n  merchant_id,\n  game_id,\n  game_code,\n  result,\n  currency,\n  ROUND(CAST(amount AS DOUBLE), 2) AS amount,\n  ROUND(CAST(pay_out AS DOUBLE), 2) AS pay_out,\n  multiplier,\n  balance,\n  detail,\n  DATE_FORMAT(FROM_UNIXTIME(created_at / 1000), '%Y-%m-%d %H:%i:%s') AS created_at,\n  DATE_FORMAT(FROM_UNIXTIME(updated_at / 1000), '%Y-%m-%d %H:%i:%s') AS updated_at\nFROM game_records\nWHERE\n  provider = 'gp'\n  AND merchant = '1754913084'\n  AND updated_at BETWEEN \n    TO_UNIXTIME(PARSE_DATETIME('2025-10-16 00:00:00', 'yyyy-MM-dd HH:mm:ss')) * 1000\n    AND TO_UNIXTIME(PARSE_DATETIME('2025-10-16 23:59:59', 'yyyy-MM-dd HH:mm:ss')) * 1000\nLIMIT 100",
  "knowledgeBaseUsed": "否",
  "merchantDataUsed": "是"
}
```
