// 灵活版账单金额匹配节点 - 支持所有支付状态的账单
const inputData = $input.all();

console.log('=== 开始灵活版账单金额匹配 ===');
...
function findBestMatch(billAmount, billMerchant, transferData) {
  ...
  for (const transfer of transferData) {
    ...
    if (amountDiff < 10) {
      const confidence = Math.max(50, 100 - (amountDiff * 5));
  ...
  for (const transfer of transferData) {
    ...
    if (percentageDiff < 10 && amountDiff < 100) {
      const confidence = Math.max(30, 100 - (percentageDiff * 8));
      if (confidence > bestConfidence) {
        bestMatch = {
          transfer,
          amountDiff,
...
billingData.forEach((bill, billIndex) => {
  ...
  const paymentStatus = bill['是否支付'] || '未知';
  
  console.log(`\n处理账单 ${billIndex + 1}: ${billMerchant}, 金额: ${billAmount}, 支付状态: "${paymentStatus}"`);
  ...
  if (bestMatch && bestMatch.confidence >= 30) {
    ...
    // 根据支付状态确定状态
    let finalPaymentStatus = '已支付';
    if (paymentStatus === '') {
      finalPaymentStatus = '待确认';
    } else if (paymentStatus === '否') {
      finalPaymentStatus = '未支付';
    }
