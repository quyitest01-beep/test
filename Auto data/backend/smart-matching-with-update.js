// 智能匹配节点 - 找到匹配时自动更新"是否支付"状态
const inputData = $input.all();

console.log('=== 开始智能匹配与状态更新 ===');
...
function findBestMatch(billAmount, billMerchant, transferData) {
  ...
  for (const transfer of transferData) {
    ...
    if (amountDiff < 5) {
      const confidence = Math.max(70, 100 - (amountDiff * 6));
      if (confidence > bestConfidence) {
        bestMatch = {
          ...
          strategy: '固定阈值匹配',
          shouldUpdatePaymentStatus: confidence >= 80
        };
  ...
  for (const transfer of transferData) {
    ...
    if (percentageDiff < 3 && amountDiff < 50) {
      const confidence = Math.max(60, 100 - (percentageDiff * 12));
      if (confidence > bestConfidence) {
        bestMatch = {
          transfer,
          amountDiff,
...
const updatedBills = []; // 记录需要更新支付状态的账单
...
billingData.forEach((bill, billIndex) => {
  ...
  if (bestMatch && bestMatch.confidence >= 60) {
    ...
    // 如果找到匹配且置信度足够高，自动更新支付状态
    if (bestMatch.shouldUpdatePaymentStatus && originalPaymentStatus !== '是') {
    ...
    matchingResults.push({
      ...
      should_update_payment_status: bestMatch.shouldUpdatePaymentStatus
    });
  } else {
...
const sheetsOutputData = matchingResults.map((result, index) => ({
  ...
  '应更新支付状态': result.should_update_payment_status ? '是' : '否',
...
console.log(`其中 ${statusUpdatedBills} 条记录的支付状态已自动更新`);
