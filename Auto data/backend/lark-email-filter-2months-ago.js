...
// 计算上上月（往前推两个月）
function getPrevPrevMonthMeta(ref = new Date()) {
  let y = ref.getFullYear();
  let m = ref.getMonth(); // 0..11（当前月）
  ...
  const mm = pad2(m + 1); // 加1是因为getMonth()返回0-11
  return {
    year: y,
    month: Number(mm),
    yearMonth: `${y}${mm}`,
...
const targetMonthMeta = getPrevPrevMonthMeta(now);
