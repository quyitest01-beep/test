// n8n Function节点：修正版Lark子表创建器
// 参考正确的处理方式，直接构建HTTP请求体
...
async function execute() {
  try {
    ...
    larkConfigs.forEach((config, index) => {
      ...
      // 准备写入Lark的数据 - 直接合并headers和rows
      const combinedData = [config.larkData.headers, ...config.larkData.rows];
      console.log(`合并后的数据行数: ${combinedData.length}`);
      console.log(`第一行（标题）: ${JSON.stringify(combinedData[0])}`);
      console.log(`第二行（数据）: ${JSON.stringify(combinedData[1])}`);
      
      // 构建完整的HTTP请求体 - 参考正确的方式
      const httpRequest = {
        method: "POST",
        url: `https://open.larksuite.com/open-apis/sheets/v2/spreadsheets/${config.spreadsheetToken}/values_batch_update`,
        headers: {
        ...
        body: JSON.stringify({
          valueRanges: [
            {
              range: `${sheetId || 'DJYJy'}!A1:H${config.dataCount + 1}`,
              values: combinedData
            }
          ]
        })
      ...
      console.log(`HTTP请求体:`, JSON.stringify(httpRequest, null, 2).substring(0, 500) + "...");
