// n8n Code节点：处理 Loop Over Items 输出，提取游戏名称和 sheetId
// 功能：
// 1. 从 data.valueRange.range 中提取 sheetId（如 "3vrYm4!A1:O100" -> "3vrYm4"）
// 2. 从 data.valueRange.values 中提取游戏名称（从表格数据的"游戏名称"列）
// 3. 为每个输入项输出一个结果
...
// 收集 token（从第一个输入项中提取，或从所有输入项中查找）
...
inputs.forEach((wrapper, index) => {
  ...
  console.log(`\n📦 处理输入项 [${index}]:`);
  console.log(`  - 是否有 data.valueRange: ${!!item.data?.valueRange}`);
  console.log(`  - 是否有 data.valueRange.range: ${!!item.data?.valueRange?.range}`);
  console.log(`  - 是否有 data.valueRange.values: ${!!item.data?.valueRange?.values}`);
  
  // 提取 token（从第一个输入项或包含 token 的输入项中提取）
  if (index === 0 || !tenantToken) {
  ...
  if (!spreadsheetToken) {
    ...
    if (spreadsheetToken) {
      console.log(`  ✅ 提取到 spreadsheet_token: ${spreadsheetToken}`);
    }
  }
  ...
  if (!range) {
    console.warn(`  ⚠️ 输入项 [${index}] 没有 data.valueRange.range`);
    return;
  }
  ...
  if (!gameName) {
    ...
    // 即使没有游戏名称，也输出结果（使用 sheetId 作为游戏名称）
    const fallbackGameName = sheetId;
    console.log(`  💡 使用 sheetId 作为游戏名称: "${fallbackGameName}"`);
    
    const result = {
      tenant_access_token: tenantToken,
      spreadsheet_token: spreadsheetToken || item.data?.spreadsheetToken || null,
      sheetId: sheetId,
      target_game: fallbackGameName,
      sheets: [{
        sheetId: sheetId,
  ...
  const result = {
    ...
    sheets: [{
      sheetId: sheetId,
      title: null, // 输出中没有 title，但可以从游戏名称推断
      range: range
    }],
    // 保留原始数据中的其他字段
    code: item.code,
...
console.log(`\n=== 处理完成 ===`);
console.log(`共处理 ${inputs.length} 个输入项，成功提取: ${results.length} 个结果`);
...
if (results.length > 0) {
  ...
  console.error(`\n❌ 没有提取到任何结果！`);
  console.error(`请检查输入数据格式是否正确。`);
  console.error(`期望格式: { data: { valueRange: { range: "sheetId!A1:O100", values: [...] } } }`);
}
