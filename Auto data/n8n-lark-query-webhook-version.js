// n8n Code节点：Lark消息查询处理器 - Webhook专用版本
// 专门用于Webhook回复，不需要message_id等参数
  ...
  try {
    ...
    results.push({
      json: {
        ...
        // Lark Webhook消息格式（直接用于HTTP Request）
        larkMessage: {
          msg_type: "text",
          content: {
            text: replyMessage
...
// Webhook专用版本特点：
// 1. 只输出2个字段：replyMessage 和 larkMessage
// 2. 移除了larkParams和larkReply相关逻辑
// 3. 专门用于Webhook回复场景
// 4. 使用方法：HTTP Request Body设置为 {{ $json.larkMessage }}
// 5. 支持双向查询和中文格式
// 6. 简化输出，专注于消息内容