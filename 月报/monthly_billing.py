    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
            ...
            columns = [
                ...
                ('E:E', 12, num_format_6),  # USD汇率（6位小数）
        ...
        provider_grouped['USD汇率'] = provider_grouped['currency'].map(exchange_rate_dict).round(6)
        ...
        with pd.ExcelWriter(provider_output_file, engine='xlsxwriter') as writer:
            ...
            worksheet.set_column('D:D', 12, num_format_6)  # USD汇率（6位小数）
        ...
        provider_game_grouped['USD汇率'] = provider_game_grouped['currency'].map(exchange_rate_dict).round(6)
...
def parse_args():
    ...
    parser.add_argument('--date', type=str, default=None, help='定时任务传递的时间，格式如2024-06')
...
def new_monthly_billing(month, mode='manual'):
    """主函数"""
    ...
    try:
        # 解析参数，确定月份
        # args = parse_args()
        args = argparse.Namespace(mode=mode, date=month)
